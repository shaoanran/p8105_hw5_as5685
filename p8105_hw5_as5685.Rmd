...
mutate(
  beta_lists = map(.x = betas, ~rerun(100, sim_regression( beta1 = .x))),
...
#### Plot for true betas and estimated betas as well asn estimated betas with p-values less than 0.05. 
```{r}
plot_beta = 
  sim_betas %>% 
  group_by(betas) %>% 
